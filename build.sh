...
build() {
    ...
    cmake -Wno-dev --fresh -DTARGET_ARCH=$arch-w64-mingw32 $gcc_arch -DCOMPILER_TOOLCHAIN=$compiler "${clang_option[@]}" "${pgo_option[@]}" $extra_option -DENABLE_CCACHE=ON -DSINGLE_SOURCE_LOCATION=$srcdir -DRUSTUP_LOCATION=$buildroot/install_rustup -G Ninja -H$gitdir -B$buildroot/build$bit
    ...
    if [[ ! "$(ls -A $buildroot/install_rustup/bin)" ]]; then
    ...
    ninja -C $buildroot/build$bit svtav1-psy
    ...
    if [ "$arch" == "x86_64" ]; then
        ...
        sudo wine ./minject.exe $buildroot/build$bit/install/$arch-w64-mingw32/bin/ffplay.exe --inplace -y
